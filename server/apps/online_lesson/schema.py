...
class Online_lesson_paginationType(graphene.ObjectType):
    page = graphene.Int()
    per_page = graphene.Int()
    page_count = graphene.Int()
    total_count = graphene.Int()
    records = graphene.List(Online_lessonType)
...
class Query(object):
    ...
    all_online_lessons_pagination = graphene.Field(
        Online_lesson_paginationType,
        page=graphene.Int(),
        per_page=graphene.Int()
    )
    ...
    def resolve_all_online_lessons_pagination(self, info, page, per_page):
        ...
        if info.context.user.is_teacher==True:
            query_set = Online_lesson.objects.filter(Q(create_userID=info.context.user))
        elif info.context.user.is_student==True:
            student = Student.objects.get(user=info.context.user)
            query_set = Online_lesson.objects.filter(Q(create_userID=student.section.teacher.user, status='OPEN'))
        else:
            query_set = Online_lesson.objects.all()
        
        online_lessons = custom_paginate(
