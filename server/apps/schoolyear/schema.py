...
class CreateSchoolyear(graphene.Mutation):
    ...
    def mutate(self, info, schoolyear, start_date, end_date, is_current):

        create_userID_i = info.context.user

        schoolyear = Schoolyear(schoolyear=schoolyear, start_date=start_date,end_date=end_date, is_current=is_current, create_userID=create_userID_i)
        schoolyear.save()
