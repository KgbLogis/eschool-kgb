...
class Query(object):
    ...
    def resolve_routines(root, info, filter):
        ...
        if info.context.user.is_teacher==True:
            ...
            return Routine.objects.filter(Q(teacher=teacher_o), Q(schoolyear_id=schoolyear), Qr)
            
        if info.context.user.is_student==True:
            student = Student.objects.get(user=info.context.user)
            return Routine.objects.filter(Q(section=student.section), Q(schoolyear_id=schoolyear), Qr)
        else:
