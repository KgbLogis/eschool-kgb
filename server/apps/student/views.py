...
def Import_csv(request):
    ...
    try:
        if request.method == 'POST' and request.FILES['myfile']:
            ...
            print(excel_file) 
            empexceldata = pd.read_csv("."+excel_file,encoding='utf-8', converters={'username': str, 'password': str, 'student_code': str})
            ...
            for dbframe in dbframe.itertuples():
                ...
                userob = get_user_model()(username=dbframe.username,email="test@tse.com",first_name=dbframe.family_name,last_name=dbframe.name,is_student=True,is_teacher=False,)
                ...
                stu = Student(user=user_i, student_code=dbframe.student_code, registerNo=registerNo, religion=dbframe.religion, surname=dbframe.surname, family_name=dbframe.family_name, name=dbframe.name, nationality=dbframe.nationality, state=dbframe.state, phone=phoner1, address=dbframe.address, join_date=dbframe.join_date, join_schoolyear=join_schoolyear_i, sex=dbframe.sex, classtime=classtime_i, status=status_i, status_extra=status_extra_i, activity=activity_i,  birthdate="2000-01-01", birth_city=birth_city_i, birth_district=birth_district_i, school=school_i, program=program_i, classes=classes_i, section=section_i, create_userID=create_userID_i)
