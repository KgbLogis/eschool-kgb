...
class Query(object):
    ...
    def resolve_all_questions_by_test(self, info, id):
        if info.context.user.is_student==True:
            ...
            return Question.objects.filter(online_test=online_test_i)
...
class CreateQuestion(graphene.Mutation):
    ...
    def mutate(self, info, online_test, question_level, question_l, hint, image, answer_type):
        ...
        question_o = Question(online_test=online_test_i, question_level=question_level_i, question=question_l, hint=hint, image=image, answer_type=answer_type)
        question_o.save()
    ...
    def mutate(self, info, student_code, section, take_test):
        ...
        if(student_code==''):
            for student in Student.objects.filter(section_id=section):
                participant_o = Participant(student=student, take_test_id=take_test)
                participant_o.save()
        else:
            student_i = Student.objects.get(student_code=student_code)
            participant_o = Participant(student=student_i, take_test_id=take_test)
            participant_o.save()
