...
function StudentsTable(props) {
	...
	const { loading, refetch } = useQuery(ALL_STUDENTS, {
		variables: { filter: filter, program: program, classes: classes, section: section },
		onCompleted: data => {
	...
	}, [fetchProgram, user.isStudent, user.isTeacher])
	...
	const tableColumns = [
		...
		},
		{
			...
			dataIndex: 'isPaid',
			sorter: (a, b) => utils.antdTableSorter(a, b, 'isPaid'),
			render: (_, elm) => (
				<span className={classNames(
					elm.isPaid === true ? 'bg-green-400' : 'bg-red-400',
