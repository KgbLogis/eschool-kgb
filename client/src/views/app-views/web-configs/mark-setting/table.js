...
import MarkSettingForm from './form'
import { useQuery, useMutation } from '@apollo/client';
import { ALL_MARK_SETTING, DELETE_MARK_SETTING } from 'graphql/mark';
import IntlMessage from 'components/util-components/IntlMessage';

function  MarkSettingTable (props) {

	const { loading, data: markSettingsData } = useQuery(ALL_MARK_SETTING, {
	});
    
	const [list, setList] = useState(undefined)

    useEffect(() => {
        if(loading === false && markSettingsData){
            setList(markSettingsData.allMarkSettings);
        }
    }, [loading, markSettingsData])
	...
	const [deleteSetting, { loading: loadingDelete }] = useMutation(DELETE_MARK_SETTING, {
	...
	const onSearch = e => {
		...
		const searchArray = e.currentTarget.value? list : markSettingsData.allMarkSettings
	...
	return (
		<>
			<Modal
				...
				okButtonProps={{form:'MarkSettingForm', key: 'submit', htmlType: 'submit'}}
            >
                <MarkSettingForm editData={editData} formType={formType} setIsModalVisible={setIsModalVisible} />
            </Modal>
			<Card>
...
export default MarkSettingTable
