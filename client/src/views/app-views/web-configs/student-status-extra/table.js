...
import { ALL_STUDENT_STATUS_EXTRA, DELETE_STUDENET_STATUS_EXTRA } from 'graphql/core'
import IntlMessage from 'components/util-components/IntlMessage';

function  StatusTable (props) {

	const { loading, error, data: statusData } = useQuery(ALL_STUDENT_STATUS_EXTRA, {
	});
    
	const [list, setList] = useState(undefined)

    useEffect(() => {
        if(loading === false && statusData){
            setList(statusData?.allStudentStatusExtras);
        }
	...
	const [deleteActivity, { loading: loadingDelete }] = useMutation(DELETE_STUDENET_STATUS_EXTRA, {
	...
	function deleteRow(row) {
		confirm({
		  ...
		  cancelText: 'Болих',
		  onOk() {
			deleteActivity({ variables: { id: row.id} }); 
		  },
		});
	}
	...
	const onSearch = e => {
		...
		const searchArray = e.currentTarget.value? list : statusData?.allDegrees
	...
	return (
		<>
			<Modal
				...
				okButtonProps={{form:'StatusForm', key: 'submit', htmlType: 'submit'}}
            >
                <StatusForm formType={formType} editData={editData} setIsModalVisible={setIsModalVisible} />
            </Modal>
			<Card>
