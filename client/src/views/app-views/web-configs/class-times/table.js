...
import ClassTimeForm from './form'
import { useQuery, useMutation } from '@apollo/client';
import { ALL_CLASSTIME, DELETE_SCLASSTIME } from 'graphql/core'
import IntlMessage from 'components/util-components/IntlMessage';

function  ClassTimeTable (props) {

	const { loading, data: activityData } = useQuery(ALL_CLASSTIME, {
	});
    
	const [list, setList] = useState(undefined)

    useEffect(() => {
        if(loading === false && activityData){
            setList(activityData?.allClasstimes);
        }
	...
	const [deleteActivity, { loading: loadingDelete }] = useMutation(DELETE_SCLASSTIME, {
	...
	const onSearch = e => {
		...
		const searchArray = e.currentTarget.value? list : activityData?.allDegrees
	...
	return (
		<>
			<Modal 
				...
				okButtonProps={{form:'ClassTimeForm', key: 'submit', htmlType: 'submit'}}
            >
                <ClassTimeForm formType={formType} editData={editData} setIsModalVisible={setIsModalVisible} />
            </Modal>
			<Card>
