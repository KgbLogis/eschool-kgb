...
import TestLevelForm from './form'
import { useQuery, useMutation } from '@apollo/client';
import { ALL_QUESTION_LEVELS, DELETE_QUESTION_LEVEL } from 'graphql/test';
import IntlMessage from 'components/util-components/IntlMessage';

function  TestLevelTable (props) {

	const { loading, error, data, refetch } = useQuery(ALL_QUESTION_LEVELS, {
	});
    ...
    useEffect(() => {
        if(loading === false && data){
            setList(data.allQuestionLevels);
        }
    }, [loading, data, error])
	...
	const [deleteDegree, { loading: loadingDelete }] = useMutation(DELETE_QUESTION_LEVEL, {
		onCompleted: data => {
			refetch();
	...
	const onSearch = e => {
		...
		const searchArray = e.currentTarget.value? list : data?.allQuestionLevels
	...
	return (
		<>
			<Modal
				...
				okButtonProps={{form:'TestLevelForm', key: 'submit', htmlType: 'submit'}}
            >
                <TestLevelForm editData={editData} formType={formType} setIsModalVisible={setIsModalVisible} />
            </Modal>
			<Card>
